...
- 布局调整：1–5 种语言显示为一列，6–9 种语言自动切换为两列。
- 语言汇总：最多展示 10 项，第 10 项以 `Other` 汇总其余语言。
...
- 完整版本标签：可从 [Releases](https://github.com/KrelinnBios/GitHubProfileLanguageDonut/releases) 选择并固定，升级时由使用者决定。
...
- Action 只读取 GitHub API 返回的公开仓库和语言数据，不上传或保存额外的个人资料。
- 生成的 SVG 和 README 修改由使用者自己的工作流提交到自己的仓库。
- 私有仓库不会因本 Action 的公开仓库查询而写入生成图表。

## 内容边界

- 图表反映公开仓库当前代码体量的语言构成，不代表能力、熟练度、投入时间或个人身份。
- GitHub、仓库名称、语言数据和生成图表中的其他第三方内容分别适用其权利人的条款与权利声明。
...
欢迎通过 [GitHub Issues](https://github.com/KrelinnBios/GitHubProfileLanguageDonut/issues) 提交使用问题、布局兼容问题、新语言配色建议、文档改进或其他功能建议。
