...
- 版面調整：1–5 種語言顯示為一欄，6–9 種語言自動切換為兩欄。
- 語言彙總：最多呈現 10 項，第 10 項以 `Other` 彙總其餘語言。
...
<p align="center">
  <img src="examples/preview-contiguous-20260720.svg" alt="語言佔比環形圖預覽">
</p>
...
定時更新無需設定任何跨儲存庫 Token，語言資料變化不頻繁時完全夠用。如需更高即時性，可手動點選 **Run workflow** 立即更新。
...
- Action 只讀取 GitHub API 回傳的公開儲存庫和語言資料，不會上傳或保存額外的個人資料。
- 產生的 SVG 和 README 修改由使用者自己的工作流程提交到自己的儲存庫。
- 私有儲存庫不會因本 Action 的公開儲存庫查詢而寫入產生圖表。

## 內容邊界

- 圖表反映公開儲存庫目前程式碼量的語言構成，不代表能力、熟練度、投入時間或個人身分。
- GitHub、儲存庫名稱、語言資料和產生圖表中的其他第三方內容，分別適用其權利人的條款與權利聲明。
- 使用者應自行確認權杖權限、產生內容和自動提交行為符合所在組織及儲存庫規則。

## 授權條款

本專案依據 [MIT License](./LICENSE) 釋出，允許使用、修改、散布和商業用途，但須保留授權條款與版權宣告。GitHub 資料、GitHub Actions、第三方軟體、外部服務和使用者產生的內容，不會僅因本專案讀取、呼叫或產生而自動納入 MIT 授權，詳見 [THIRD-PARTY-NOTICES.md](./THIRD-PARTY-NOTICES.md)。
