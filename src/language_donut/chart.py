...
def full_ring_path(center_x, center_y, inner_radius, outer_radius, start_angle):
    """Return a full annulus path using two half-circle arcs."""
    mid_angle = start_angle + math.pi

    outer_start_x, outer_start_y = polar_to_cartesian(
        center_x, center_y, outer_radius, start_angle
    )
    outer_mid_x, outer_mid_y = polar_to_cartesian(
        center_x, center_y, outer_radius, mid_angle
    )
    inner_start_x, inner_start_y = polar_to_cartesian(
        center_x, center_y, inner_radius, start_angle
    )
    inner_mid_x, inner_mid_y = polar_to_cartesian(
        center_x, center_y, inner_radius, mid_angle
    )

    return (
        f"M {outer_start_x:.2f} {outer_start_y:.2f} "
        f"A {outer_radius:.2f} {outer_radius:.2f} 0 1 1 "
        f"{outer_mid_x:.2f} {outer_mid_y:.2f} "
        f"A {outer_radius:.2f} {outer_radius:.2f} 0 1 1 "
        f"{outer_start_x:.2f} {outer_start_y:.2f} "
        f"L {inner_start_x:.2f} {inner_start_y:.2f} "
        f"A {inner_radius:.2f} {inner_radius:.2f} 0 1 0 "
        f"{inner_mid_x:.2f} {inner_mid_y:.2f} "
        f"A {inner_radius:.2f} {inner_radius:.2f} 0 1 0 "
        f"{inner_start_x:.2f} {inner_start_y:.2f} Z"
    )


def annular_segment_path(
    center_x,
    center_y,
    inner_radius,
    outer_radius,
    ...
    """Return one contiguous filled donut segment with curved dividers."""
    ...
    outer_end_x, outer_end_y = polar_to_cartesian(
        center_x, center_y, outer_radius, end_angle
    )
    inner_start_x, inner_start_y = polar_to_cartesian(
        center_x, center_y, inner_radius, start_angle
    )
    inner_end_x, inner_end_y = polar_to_cartesian(
        center_x, center_y, inner_radius, end_angle
    )
    end_control_x, end_control_y = divider_control_point(
        center_x,
        center_y,
        inner_radius,
        outer_radius,
    ...
    start_control_x, start_control_y = divider_control_point(
        center_x,
        center_y,
        inner_radius,
        outer_radius,
    ...
    return (
        ...
        f"{outer_end_x:.2f} {outer_end_y:.2f} "
        f"Q {end_control_x:.2f} {end_control_y:.2f} "
        f"{inner_end_x:.2f} {inner_end_y:.2f} "
        f"A {inner_radius:.2f} {inner_radius:.2f} 0 {large_arc} 0 "
        f"{inner_start_x:.2f} {inner_start_y:.2f} "
        f"Q {start_control_x:.2f} {start_control_y:.2f} "
        f"{outer_start_x:.2f} {outer_start_y:.2f} Z"
    )
...
def build_svg(totals, config):
    ...
    column_count = min(
        max_columns,
        max(1, math.ceil(len(items) / rows_per_column)),
    )
    ...
    center_y = height / 2
    center_radius = min(float(chart["donut_radius"]), (height - 36) / 2)
    ring_width = float(chart["donut_width"])
    outer_radius = center_radius + ring_width / 2
    inner_radius = max(0.0, center_radius - ring_width / 2)
    ...
    for index, ((language, byte_count), display_percentage) in enumerate(
        ...
        if path:
            segments.append(
                ...
                f'fill="{color_for(language, colors)}" />'
            )
    ...
    for index, (language, byte_count) in enumerate(items):
        ...
        row = [
            f'    <circle cx="{column_x + 6:.1f}" cy="{y - 4:.1f}" '
            f'r="5" fill="{color}"/>',
            f'    <text class="label" x="{label_x:.1f}" '
            f'y="{y:.1f}">{html.escape(language)}</text>',
            f'    <text class="percent" x="{percent_x:.1f}" '
            f'y="{y:.1f}" text-anchor="end">'
            f'{percentage_label(value)}</text>',
        ]

        if show_bars:
            row.extend(
                [
                    f'    <rect class="bar-track" x="{label_x:.1f}" '
                    f'y="{y + 9:.1f}" width="{bar_width:.1f}" '
                    f'height="3" rx="1.5"/>',
                    f'    <rect x="{label_x:.1f}" y="{y + 9:.1f}" '
                    f'width="{max(4.0, bar_width * value / 100):.2f}" '
    ...
    if chart["show_center_label"]:
        center_text = (
            ...
            f'y="{center_y - 2:.1f}">'
            f'{percentage_label(dominant_percentage)}</text>\n'
            f'  <text class="center-label" x="{center_x:.1f}" '
            f'y="{center_y + 19:.1f}">'
            f'{html.escape(dominant_language)}</text>\n'
        )
